...
with col_input:
    ...
    if missing_secrets:
        st.error(
            ...
            + ". Add these values to your local .env file or deployment secrets."
        )
...
if st.session_state.running and not st.session_state.done:
    # Load the heavier AI dependencies only after the user starts a research run.
    ...
    with st.spinner("📄  Reader Agent is scraping top resources…"):
        urls = re.findall(r"URL:\s*(https?://\S+)", results["search"])
        results["reader"] = (
            scrape_url.invoke(urls[0])
            if urls
            else "No URL was available to scrape."
        )
        st.session_state.results = dict(results)
...
if r:
    ...
    # Final report
    if "writer" in r:
        report_title = st.session_state.get("topic_input", "Research Topic").strip()
        full_report = f"# Research Report: {report_title}\n\n{r['writer']}"
        st.markdown("""
